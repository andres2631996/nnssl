...
from nnssl.training.loss.mse_loss import (
    ...
    AnatDistWeightedMAEMSELoss_recDistMap,
...
def create_blocky_mask_segm(
    tensor_size,
    block_size,
    seg=None,
    sparsity_factor : float = 0.75,
    lambda_seg : float = 0.7,
    rng_seed: int | None = None,
):
    """
    Simplified anatomy-guided MAE mask.

    - keeps global mask ratio fixed (sparsity_factor = ρ)
    - biases masking toward vessel-containing tokens
    """

    small_mask_size = tuple(size // block_size for size in tensor_size)
    n_tokens = np.prod(small_mask_size)
    n_masked = int(sparsity_factor * n_tokens)

    flat_mask = torch.ones(n_tokens)
    ...
    # ---------------------------------------------------------
    # fallback: standard MAE masking
    # ---------------------------------------------------------
    if seg is None:
        idx = torch.randperm(n_tokens, generator=gen)[:n_masked]
        flat_mask[idx] = 0
        return flat_mask.reshape(small_mask_size)

    # ---------------------------------------------------------
    # anatomy-guided masking
    # ---------------------------------------------------------

    seg = seg.squeeze().float()

    # pool to token grid
    v = seg.reshape(
        small_mask_size[0], block_size,
        small_mask_size[1], block_size,
        small_mask_size[2], block_size,
    ).mean((1, 3, 5)).flatten()

    # binary -> occupancy already in [0,1]

    # paper-like normalization (IMPORTANT)
    v_norm = (v - v.mean()) / (v.std() + 1e-6)

    # π_i = σ(λ_v * ṽ_i)
    probs = torch.sigmoid(-lambda_seg * v_norm)

    # enforce exact masking ratio via multinomial sampling
...
class MaskMAETrainer(BaseMAETrainer):
    @staticmethod
    def mask_creation(
        segm: torch.Tensor,
        batch_size: int,
    ...
    def train_step(self, batch: dict) -> dict:
        data = batch["data"]
        data = data.to(self.device, non_blocking=True)
        segm = batch["seg"].to(self.device, non_blocking=True)

        # We use the self.batch_size as it is not identical with the plan batch_size in ddp cases.
        mask = self.mask_creation(
            segm, self.batch_size, self.config_plan.patch_size, self.mask_percentage
        ).to(self.device, non_blocking=True)
        # Make the mask the same size as the data
        rep_D, rep_H, rep_W = (
            data.shape[2] // mask.shape[2],
...
class MaskMAETrainer_ANAT(MaskMAETrainer):
...
class MaskMAETrainer_ANAT_AMAPLoss(MaskMAETrainer_ANAT):
    def build_loss(self):
        """
        This is where you build your loss function. You can use anything from torch.nn here.
        In general the MAE losses are only applied on regions where the mask is 0.
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        # We use the self.batch_size as it is not identical with the plan batch_size in ddp cases.
        mask = self.mask_creation(
            anat, self.batch_size, self.config_plan.patch_size, self.mask_percentage
        ).to(self.device, non_blocking=True)
        # Make the mask the same size as the data
        rep_D, rep_H, rep_W = (
            data.shape[2] // mask.shape[2],
...
############################# WARMUP 50 EPOCHS #############################
class MaskMAETrainer_ANAT_warmup50ep(MaskMAETrainer_ANAT):
    ...
    def __init__(
        ...
        super(MaskMAETrainer_ANAT_warmup50ep, self).__init__(
            plan,
            configuration_name,
            fold,
            pretrain_json,
...
class MaskMAETrainer_ANAT_AMAPLoss_warmup50ep(MaskMAETrainer_ANAT_AMAPLoss):
    def build_loss(self):
        """
        This is where you build your loss function. You can use anything from torch.nn here.
        In general the MAE losses are only applied on regions where the mask is 0.
    ...
    def __init__(
        ...
        super(MaskMAETrainer_ANAT_AMAPLoss_warmup50ep, self).__init__(
            plan,
            configuration_name,
            fold,
            pretrain_json,
...
class MaskMAETrainer_ANAT_weightedANAT(MaskMAETrainer_ANAT):
...
class BaseMAETrainer_weightedANATCorrupted(BaseMAETrainer):
    ...
    def get_dataloaders(self):
        ...
        dl_tr, dl_val = self.get_dist_corrupted_dataloaders(initial_patch_size)
...
class BaseMAETrainer_weightedANATCorruptedElastic(BaseMAETrainer_weightedANATCorrupted):
    ...
    def get_dataloaders(self):
        ...
        dl_tr, dl_val = self.get_dist_corrupted_elastic_dataloaders(initial_patch_size)
...
class BaseMAETrainer_recDistMap(BaseMAETrainer):
    ...
    def __init__(
        ...
        self.num_output_channels = 2 # Set a second channel for distance map estimation
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with (
            ...
            output = self.network(masked_data)
            # del data
            l = self.loss(output, data, dist, mask)
...
class BaseMAETrainer_recDistMap2chan(BaseMAETrainer):
    ...
    def __init__(
        ...
        self.config_plan.patch_size = (160, 160, 160)
        self.mask_percentage: float = 0.75
        self.num_input_channels = 2 # Set a second input channel for distance map inputs
...
class BaseMAETrainer_weightedANAT_recDistMap(BaseMAETrainer_recDistMap):
    ...
    def build_loss(self):
        ...
        :return:
        """
        return AnatDistWeightedMAEMSELoss_recDistMap()
...
class BaseMAETrainer_recDistMap2chan_warmup50ep(BaseMAETrainer_recDistMap2chan):
    ...
    def __init__(
        ...
        super(BaseMAETrainer_recDistMap2chan_warmup50ep, self).__init__(
            plan,
            configuration_name,
            fold,
            pretrain_json,
...
class BaseMAETrainer_recDistMap_warmup50ep(BaseMAETrainer_recDistMap):
    ...
    def __init__(
        ...
        super(BaseMAETrainer_recDistMap_warmup50ep, self).__init__(
            plan,
            configuration_name,
            fold,
            pretrain_json,
...
class BaseMAETrainer_weightedANAT_recDistMap_warmup50ep(BaseMAETrainer_weightedANAT_recDistMap):
    ...
    def __init__(
        ...
        super(BaseMAETrainer_weightedANAT_recDistMap_warmup50ep, self).__init__(
            plan,
            configuration_name,
            fold,
            pretrain_json,
...
class MaskMAETrainer_ANAT_recDistMap(MaskMAETrainer_ANAT):
...
class MaskMAETrainer_ANAT_weightedANAT_recDistMap(MaskMAETrainer_ANAT_recDistMap):
...
class MaskMAETrainer_ANAT_weightedANAT_recDistMap_warmup50ep(MaskMAETrainer_ANAT_weightedANAT_recDistMap):
    ...
    def __init__(
        ...
        super(MaskMAETrainer_ANAT_weightedANAT_recDistMap_warmup50ep, self).__init__(
            plan,
            configuration_name,
            fold,
            pretrain_json,
...
class MaskMAETrainer_ANAT_recDistMap_warmup50ep(MaskMAETrainer_ANAT_recDistMap):
    ...
    def __init__(
        ...
        super(MaskMAETrainer_ANAT_recDistMap_warmup50ep, self).__init__(
            plan,
            configuration_name,
            fold,
            pretrain_json,
...
class MaskMAETrainer_ANAT_weightedANAT_warmup50ep(MaskMAETrainer_ANAT_weightedANAT):
    ...
    def __init__(
        ...
        super(MaskMAETrainer_ANAT_weightedANAT_warmup50ep, self).__init__(
            plan,
            configuration_name,
            fold,
            pretrain_json,
...
class BaseMAETrainer_weightedANATCorrupted_warmup50ep(BaseMAETrainer_weightedANATCorrupted):
    ...
    def __init__(
        ...
        super(BaseMAETrainer_weightedANATCorrupted_warmup50ep, self).__init__(
            plan,
            configuration_name,
            fold,
            pretrain_json,
...
class BaseMAETrainer_weightedANATCorruptedElastic_warmup50ep(BaseMAETrainer_weightedANATCorruptedElastic):
    ...
    def __init__(
        ...
        super(BaseMAETrainer_weightedANATCorruptedElastic_warmup50ep, self).__init__(
            plan,
            configuration_name,
            fold,
            pretrain_json,
...
class BaseMAETrainer_warmup50ep_featLoss(BaseMAETrainer_warmup50ep):
    ...
    def __init__(
        ...
        super(BaseMAETrainer_warmup50ep_featLoss, self).__init__(
            plan,
            configuration_name,
            fold,
            pretrain_json,
    ...
    def initialize(self):
        super(BaseMAETrainer_warmup50ep_featLoss, self).initialize()
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=(self.device.type=="cuda")) if self.device.type=="cuda" else dummy_context():
            ...
            recon_loss = self.loss(student_out, data, mask)
